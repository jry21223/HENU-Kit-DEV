  ...
  <div class="min-h-screen bg-[var(--hk-paper)] text-[var(--hk-ink)]" data-console-shell>
    <aside class="console-sidebar" aria-label="Console 主导航">
      <div class="flex items-center gap-3 px-4 py-5">
        ...
        <div>
          <strong class="block text-sm tracking-tight text-white">HENUKit Console</strong>
          <span class="text-xs text-white/55">学生自主运营</span>
        </div>
      </div>
      ...
      <nav class="mt-4 grid gap-1 px-3" aria-label="产品模块">
        <a
          ...
          class="flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-medium text-white/70 transition hover:bg-white/10 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--hk-wheat-gold)]"
        >
      ...
      <div class="mt-auto p-4 text-xs leading-5 text-white/45">非河南大学官方项目<br />Console V1 · Mock data</div>
    </aside>
            ...
            <DialogContent class="fixed inset-y-0 left-0 z-50 w-[min(84vw,20rem)] bg-[var(--hk-ink-green-deep)] p-5 text-white shadow-2xl">
              <div class="flex items-start justify-between gap-4">
                <div>
                  ...
                  <DialogDescription class="mt-1 text-sm text-white/60">六个已确认的运营模块</DialogDescription>
                </div>
              ...
              <nav class="mt-6 grid gap-2" aria-label="移动端产品模块">
                <DialogClose v-for="module in moduleSummaries" :key="module.id" as-child>
                  <a :href="`#module-${module.id}`" class="flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm text-white/80 hover:bg-white/10">
      ...
      <main class="mx-auto w-full max-w-[var(--hk-content-max)] px-4 py-6 sm:px-6 lg:px-8 lg:py-9">
        <section class="overview-hero" aria-labelledby="overview-heading">
          <div>
            ...
            <p class="mt-2 max-w-2xl text-sm leading-6 text-[var(--hk-ink-muted)]">
              六个产品模块保持各自的数据所有权。此页面仅验证 Console 的结构、权限表达与降级状态，不连接生产服务。
            </p>
          </div>
          <div class="access-context" aria-label="当前访问上下文">
        ...
        <section class="mt-6 rounded-[var(--hk-radius-feature)] border border-[var(--hk-paper-line)] bg-white p-5 shadow-[var(--hk-shadow-card)]" aria-labelledby="permission-heading">
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p class="mt-2 max-w-2xl text-sm leading-6 text-[var(--hk-ink-muted)]">前端只呈现服务端返回的访问上下文，不信任浏览器角色，也不使用旧的单一 isAdmin 判断。</p>
            </div>
            <div class="flex flex-wrap gap-2" aria-label="有效权限">
