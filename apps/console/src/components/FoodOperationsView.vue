  ...
  <section aria-labelledby="food-heading">
    ...
    <template v-else-if="workspace">
      ...
      <div class="mt-6 grid gap-5 xl:grid-cols-3">
        <Card class="p-4" aria-labelledby="food-submissions"><h2 id="food-submissions" class="text-lg font-bold">投稿审核</h2><p v-if="!workspace.submissions.length" class="mt-3 text-muted-foreground">暂无待审核投稿。</p><ul class="mt-3 grid gap-3"><li v-for="item in workspace.submissions" :key="item.id" class="rounded-lg border p-3"><strong>{{ item.venue_name }} · {{ item.item_name }}</strong><p class="mt-1 text-sm text-muted-foreground">{{ item.description }} · v{{ item.version }}</p><div v-if="canReview && item.status === 'pending'" class="mt-3 grid gap-2"><template v-if="isConfirming(item.id, 'submission_approve', 'submission_reject')"><div class="grid gap-2 rounded-lg border p-3"><strong>{{ confirmTarget?.title }}</strong><label class="grid gap-1 text-sm">操作理由<Textarea v-model="confirmReason" maxlength="1000" rows="3" placeholder="请填写本次操作理由"></Textarea></label><div class="flex flex-wrap gap-2"><Button :disabled="busy || confirmReason.trim().length < 2" @click="submitConfirm">{{ confirmTarget?.confirmLabel }}</Button><Button variant="ghost" :disabled="busy" @click="cancelConfirm">取消</Button></div></div></template><div v-else class="flex flex-wrap gap-2"><Button :disabled="busy" @click="openSubmissionConfirm(item, true)">批准投稿</Button><Button variant="ghost" :disabled="busy" @click="openSubmissionConfirm(item, false)">拒绝投稿</Button></div></div><p v-else-if="!canReview" class="mt-3 text-sm">只读权限</p></li></ul></Card>
