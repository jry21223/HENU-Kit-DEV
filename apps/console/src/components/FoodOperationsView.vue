  ...
  <section aria-labelledby="food-heading">
    ...
    <template v-else-if="workspace">
      ...
      <div class="mt-6 grid gap-5 xl:grid-cols-3">
        <section class="operation-panel" aria-labelledby="food-submissions"><h2 id="food-submissions" class="text-lg font-bold">投稿审核</h2><p v-if="!workspace.submissions.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无待审核投稿。</p><ul class="mt-3 grid gap-3"><li v-for="item in workspace.submissions" :key="item.id" class="rounded-[4px] border p-3"><strong>{{ item.venue_name }} · {{ item.item_name }}</strong><p class="mt-1 text-sm text-[var(--hk-ink-muted)]">{{ item.description }} · v{{ item.version }}</p><div v-if="canReview && item.status === 'pending'" class="mt-3 flex flex-wrap gap-2"><UiButton @click="reviewSubmission(item, true)">批准投稿</UiButton><UiButton variant="ghost" @click="reviewSubmission(item, false)">拒绝投稿</UiButton></div><p v-else-if="!canReview" class="mt-3 text-sm">只读权限</p></li></ul></section>
        <section class="operation-panel" aria-labelledby="food-anomalies"><h2 id="food-anomalies" class="text-lg font-bold">异常票处理</h2><p v-if="!workspace.anomaly_tickets.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无异常票。</p><ul class="mt-3 grid gap-3"><li v-for="item in workspace.anomaly_tickets" :key="item.id" class="rounded-[4px] border p-3"><strong>{{ item.venue_name }} · {{ item.kind }}</strong><p class="mt-1 text-sm text-[var(--hk-ink-muted)]">{{ item.details }} · {{ item.severity }} · v{{ item.version }}</p><div v-if="canHandleAnomaly && item.status === 'open'" class="mt-3 flex flex-wrap gap-2"><UiButton @click="handleAnomaly(item, true)">标记已处理</UiButton><UiButton variant="ghost" @click="handleAnomaly(item, false)">驳回异常票</UiButton></div><p v-else-if="!canHandleAnomaly" class="mt-3 text-sm">只读权限</p></li></ul></section>
        <section class="operation-panel" aria-labelledby="food-tiers"><h2 id="food-tiers" class="text-lg font-bold">调档确认</h2><p v-if="!workspace.tier_adjustments.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无待确认调档。</p><ul class="mt-3 grid gap-3"><li v-for="item in workspace.tier_adjustments" :key="item.id" class="rounded-[4px] border p-3"><strong>{{ item.venue_name }}</strong><p class="mt-1 text-sm text-[var(--hk-ink-muted)]">{{ item.current_tier }} → {{ item.proposed_tier }} · {{ item.reason }} · v{{ item.version }}</p><div v-if="canAdjustTier && item.status === 'pending'" class="mt-3 flex flex-wrap gap-2"><UiButton @click="adjustTier(item, true)">确认调档</UiButton><UiButton variant="ghost" @click="adjustTier(item, false)">拒绝调档</UiButton></div><p v-else-if="!canAdjustTier" class="mt-3 text-sm">只读权限</p></li></ul></section>
      </div>
    </template>
  </section>
</template>
