  ...
  <section aria-labelledby="food-heading">
    ...
    <template v-else-if="workspace">
      ...
      <div class="mt-6 grid gap-5 xl:grid-cols-3">
        <Card class="p-4" aria-labelledby="food-submissions"><h2 id="food-submissions" class="text-lg font-bold">投稿审核</h2><p v-if="!workspace.submissions.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无待审核投稿。</p><ul class="mt-3 grid gap-3"><li v-for="item in workspace.submissions" :key="item.id" class="rounded-[var(--hk-radius-card)] border p-3"><strong>{{ item.venue_name }} · {{ item.item_name }}</strong><p class="mt-1 text-sm text-[var(--hk-ink-muted)]">{{ item.description }} · v{{ item.version }}</p><div v-if="canReview && item.status === 'pending'" class="mt-3 flex flex-wrap gap-2"><Button @click="reviewSubmission(item, true)">批准投稿</Button><Button variant="ghost" @click="reviewSubmission(item, false)">拒绝投稿</Button></div><p v-else-if="!canReview" class="mt-3 text-sm">只读权限</p></li></ul></Card>
        <Card class="p-4" aria-labelledby="food-anomalies"><h2 id="food-anomalies" class="text-lg font-bold">异常票处理</h2><p v-if="!workspace.anomaly_tickets.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无异常票。</p><ul class="mt-3 grid gap-3"><li v-for="item in workspace.anomaly_tickets" :key="item.id" class="rounded-[var(--hk-radius-card)] border p-3"><strong>{{ item.venue_name }} · {{ anomalyKindLabel(item.kind) }}</strong><p class="mt-1 text-sm text-[var(--hk-ink-muted)]">{{ item.details }} · {{ severityLabel(item.severity) }} · v{{ item.version }}</p><div v-if="canHandleAnomaly && item.status === 'open'" class="mt-3 flex flex-wrap gap-2"><Button @click="handleAnomaly(item, true)">标记已处理</Button><Button variant="ghost" @click="handleAnomaly(item, false)">驳回异常票</Button></div><p v-else-if="!canHandleAnomaly" class="mt-3 text-sm">只读权限</p></li></ul></Card>
