...
<template>
  <section aria-labelledby="notice-heading">
    <div class="overview-hero">
      <div><p class="eyebrow">通知流程</p><h1 id="notice-heading" class="mt-2 text-2xl font-bold sm:text-3xl">校园通知审核与分发</h1><p class="mt-2 max-w-2xl leading-7 text-[var(--hk-ink-muted)]">通知正文不可更改；审核与分发均由服务端记录。</p></div>
      <div class="access-context"><strong>{{ snapshot?.items.length ?? 0 }} 个版本</strong></div>
    </div>
    ...
    <div v-else-if="state === 'unavailable'" class="mt-6 rounded-[var(--hk-radius-card)] bg-white p-6"><p>通知服务暂时不可用，请稍后重试。</p><UiButton class="mt-4" @click="refresh">重新加载</UiButton></div>
    <div v-if="state === 'ready' && canManage" class="mt-6 grid gap-4 lg:grid-cols-2">
      <form class="rounded-[var(--hk-radius-card)] border border-[var(--hk-line)] bg-white p-5" @submit.prevent="addSource">
        ...
        <div class="mt-4 grid gap-3">
          ...
          <label class="grid gap-1 text-sm">来源主页<input v-model="sourceCanonicalURL" required type="url" pattern="https://.*" class="rounded-[var(--hk-radius-control)] border px-3 py-2" placeholder="https://example.com/notices"></label>
        </div>
      ...
      <form class="rounded-[var(--hk-radius-card)] border border-[var(--hk-line)] bg-white p-5" @submit.prevent="addVersion">
        ...
        <div class="mt-4 grid gap-3">
          <label class="grid gap-1 text-sm">来源 ID<input v-model="versionSourceID" required class="rounded-[var(--hk-radius-control)] border px-3 py-2" placeholder="来源 ID"></label>
