    ...
    <div v-else class="mt-6 grid gap-5 xl:grid-cols-[minmax(18rem,.8fr)_minmax(0,1.2fr)]">
      <form class="operation-panel !mt-0" @submit.prevent="() => lookupMembership()">
        ...
        <label class="mt-5 grid gap-2 text-sm font-semibold">
          用户 ID
          <input v-model="targetUserID" required inputmode="text" autocomplete="off" placeholder="已初始化的 UUID" :disabled="busy" class="rounded-lg border border-[var(--hk-paper-line)] bg-white px-3 py-2 font-mono text-sm font-normal" @input="resetLookup">
        </label>
      ...
      <section class="operation-panel !mt-0" :data-account-membership-detail-state="detailState" aria-labelledby="account-membership-detail-heading">
        ...
        <template v-else-if="membership">
          ...
          <form class="mt-5 border-t border-[var(--hk-paper-line)] pt-5" @submit.prevent="submitMutation">
            <label class="grid gap-2 text-sm font-semibold">
              操作原因
              <textarea v-model="reason" required maxlength="1000" rows="4" class="rounded-lg border border-[var(--hk-paper-line)] bg-white px-3 py-2 font-normal" :placeholder="membership.plan === 'free' ? '说明为何发放终身权益。' : '说明为何撤销终身权益。'"></textarea>
            </label>
