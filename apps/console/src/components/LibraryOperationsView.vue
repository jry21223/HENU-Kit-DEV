    ...
    <template v-else-if="workspace">
      ...
      <div class="mt-6 grid gap-5 xl:grid-cols-2">
        <section class="operation-panel" aria-labelledby="library-courses">
          ...
          <form v-if="canManage" class="mt-3 grid gap-2 rounded-[4px] border p-3" @submit.prevent="createCourse">
            <strong>创建课程</strong>
            <label class="grid gap-1 text-sm">学校 ID<input v-model="courseSchoolID" required type="text" class="rounded-[4px] border px-3 py-2"></label>
            <label class="grid gap-1 text-sm">学院 ID<input v-model="courseCollegeID" required type="text" class="rounded-[4px] border px-3 py-2"></label>
            <label class="grid gap-1 text-sm">专业 ID<input v-model="courseMajorID" required type="text" class="rounded-[4px] border px-3 py-2"></label>
            <label class="grid gap-1 text-sm">课程名称<input v-model="courseName" required maxlength="160" class="rounded-[4px] border px-3 py-2"></label>
            <label class="grid gap-1 text-sm">课程 slug<input v-model="courseSlug" required maxlength="160" class="rounded-[4px] border px-3 py-2"></label>
            <label class="grid gap-1 text-sm">年级<input v-model="courseGrade" maxlength="32" class="rounded-[4px] border px-3 py-2"></label>
            <UiButton type="submit">创建课程</UiButton>
          </form>
          <p v-if="!workspace.courses.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无课程。</p>
          <ul class="mt-3 grid gap-3"><li v-for="item in workspace.courses" :key="item.id" class="rounded-[4px] border p-3"><strong>{{ item.name }}</strong><p class="text-sm text-[var(--hk-ink-muted)]">{{ item.grade }} · {{ item.status }} · {{ item.slug }}</p><div v-if="canManage && item.status !== 'archived'" class="mt-3 grid gap-2"><label class="grid gap-1 text-sm">编辑课程名称<input v-model="courseEdits[item.id]" :placeholder="item.name" maxlength="160" class="rounded-[4px] border px-3 py-2"></label><div class="flex flex-wrap gap-2"><UiButton @click="updateCourse(item)">保存课程</UiButton><UiButton variant="ghost" @click="archive('course_archive', item.id, item.updated_at)">归档课程</UiButton></div></div></li></ul>
        </section>
        <section class="operation-panel" aria-labelledby="library-materials">
          <h2 id="library-materials" class="text-lg font-bold">资料</h2>
          <form v-if="canManage" class="mt-3 grid gap-2 rounded-[4px] border p-3" @submit.prevent="createMaterial">
            <strong>创建资料</strong>
            <label class="grid gap-1 text-sm">所属课程<select v-model="materialCourseID" required class="rounded-[4px] border px-3 py-2"><option value="" disabled>请选择课程</option><option v-for="course in workspace.courses" :key="course.id" :value="course.id">{{ course.name }}</option></select></label>
            <label class="grid gap-1 text-sm">资料标题<input v-model="materialTitle" required maxlength="200" class="rounded-[4px] border px-3 py-2"></label>
            <label class="grid gap-1 text-sm">存储键<input v-model="materialStorageKey" required maxlength="512" class="rounded-[4px] border px-3 py-2"></label>
            <label class="grid gap-1 text-sm">文件名<input v-model="materialFileName" required maxlength="255" class="rounded-[4px] border px-3 py-2"></label>
            <UiButton type="submit">创建资料</UiButton>
          </form>
          <p v-if="!workspace.materials.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无资料。</p>
          <ul class="mt-3 grid gap-3"><li v-for="item in workspace.materials" :key="item.id" class="rounded-[4px] border p-3"><strong>{{ item.title }}</strong><p class="text-sm text-[var(--hk-ink-muted)]">{{ item.type }} · {{ item.status }} · {{ item.file_name }}</p><div v-if="canManage && item.status !== 'archived'" class="mt-3 grid gap-2"><label class="grid gap-1 text-sm">编辑资料标题<input v-model="materialEdits[item.id]" :placeholder="item.title" maxlength="200" class="rounded-[4px] border px-3 py-2"></label><div class="flex flex-wrap gap-2"><UiButton @click="updateMaterial(item)">保存资料</UiButton><UiButton variant="ghost" @click="archive('material_archive', item.id, item.updated_at)">归档资料</UiButton></div></div></li></ul>
        </section>
        <section class="operation-panel" aria-labelledby="library-downloads"><h2 id="library-downloads" class="text-lg font-bold">下载</h2><p v-if="!workspace.downloads.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无下载记录。</p><ul class="mt-3 grid gap-3"><li v-for="item in workspace.downloads" :key="item.id" class="rounded-[4px] border p-3"><strong>{{ item.material_title }}</strong><p class="text-sm text-[var(--hk-ink-muted)]">{{ item.access_level }} · {{ new Date(item.downloaded_at).toLocaleString('zh-CN') }}</p></li></ul></section>
        <section class="operation-panel" aria-labelledby="library-submissions"><h2 id="library-submissions" class="text-lg font-bold">投稿审核</h2><p v-if="!workspace.submissions.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无待审核投稿。</p><ul class="mt-3 grid gap-3"><li v-for="item in workspace.submissions" :key="item.id" class="rounded-[4px] border p-3"><strong>{{ item.title }}</strong><p class="text-sm text-[var(--hk-ink-muted)]">{{ item.type }} · {{ item.file_name }}</p><div v-if="canReview" class="mt-3 flex flex-wrap gap-2"><UiButton @click="reviewSubmission(item, 'approve')">批准投稿</UiButton><UiButton variant="ghost" @click="reviewSubmission(item, 'reject')">拒绝投稿</UiButton></div><p v-else class="mt-3 text-sm">只读权限</p></li></ul></section>
        <section class="operation-panel xl:col-span-2" aria-labelledby="library-corrections"><h2 id="library-corrections" class="text-lg font-bold">资料纠错</h2><p v-if="!workspace.corrections.length" class="mt-3 text-[var(--hk-ink-muted)]">暂无待处理纠错。</p><ul class="mt-3 grid gap-3 md:grid-cols-2"><li v-for="item in workspace.corrections" :key="item.id" class="rounded-[4px] border p-3"><strong>{{ item.reason }}</strong><p class="mt-1 text-sm text-[var(--hk-ink-muted)]">{{ item.description }}</p><div v-if="canReview && item.status === 'pending'" class="mt-3 flex gap-2"><UiButton @click="reviewCorrection(item, 'resolve')">标记已处理</UiButton><UiButton variant="ghost" @click="reviewCorrection(item, 'reject')">驳回纠错</UiButton></div></li></ul></section>
      </div>
    </template>
  </section>
</template>
