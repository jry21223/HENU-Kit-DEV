    ...
    <template v-else-if="workspace">
      ...
      <div class="mt-6 grid gap-5 xl:grid-cols-2">
        <Card class="p-4" aria-labelledby="library-courses">
          ...
          <ul class="mt-3 grid gap-3"><li v-for="item in workspace.courses" :key="item.id" class="rounded-lg border p-3"><strong>{{ item.name }}</strong><p class="text-sm text-muted-foreground">{{ item.grade }} · {{ courseStatusLabel(item.status) }}</p><div v-if="canManage && item.status !== 'archived'" class="mt-3 grid gap-2"><label class="grid gap-1 text-sm">编辑课程名称<input v-model="courseEdits[item.id]" :placeholder="item.name" maxlength="160" class="rounded-md border px-3 py-2"></label><div class="flex flex-wrap gap-2"><Button @click="updateCourse(item)">保存课程</Button><Button variant="ghost" @click="archive('course_archive', item.id, item.updated_at)">归档课程</Button></div></div></li></ul>
        </Card>
        <Card class="p-4" aria-labelledby="library-materials">
          ...
          <ul class="mt-3 grid gap-3"><li v-for="item in workspace.materials" :key="item.id" class="rounded-lg border p-3"><strong>{{ item.title }}</strong><p class="text-sm text-muted-foreground">{{ materialTypeLabel(item.type) }} · {{ materialStatusLabel(item.status) }} · {{ item.file_name }}</p><div v-if="canManage && item.status !== 'archived'" class="mt-3 grid gap-2"><label class="grid gap-1 text-sm">编辑资料标题<input v-model="materialEdits[item.id]" :placeholder="item.title" maxlength="200" class="rounded-md border px-3 py-2"></label><div class="flex flex-wrap gap-2"><Button @click="updateMaterial(item)">保存资料</Button><Button variant="ghost" @click="archive('material_archive', item.id, item.updated_at)">归档资料</Button></div></div></li></ul>
        </Card>
        ...
        <Card class="p-4" aria-labelledby="library-submissions"><h2 id="library-submissions" class="text-lg font-bold">投稿审核</h2><p v-if="!workspace.submissions.length" class="mt-3 text-muted-foreground">暂无待审核投稿。</p><ul class="mt-3 grid gap-3"><li v-for="item in workspace.submissions" :key="item.id" class="rounded-lg border p-3"><strong>{{ item.title }}</strong><p class="text-sm text-muted-foreground">{{ item.type }} · {{ item.file_name }}</p><div v-if="canReview" class="mt-3 flex flex-wrap gap-2"><Button @click="reviewSubmission(item, 'approve')">批准投稿</Button><Button variant="ghost" @click="reviewSubmission(item, 'reject')">拒绝投稿</Button></div><p v-else class="mt-3 text-sm">只读权限</p></li></ul></Card>
        <Card class="p-4 xl:col-span-2" aria-labelledby="library-corrections"><h2 id="library-corrections" class="text-lg font-bold">资料纠错</h2><p v-if="!workspace.corrections.length" class="mt-3 text-muted-foreground">暂无待处理纠错。</p><ul class="mt-3 grid gap-3 md:grid-cols-2"><li v-for="item in workspace.corrections" :key="item.id" class="rounded-lg border p-3"><strong>{{ item.reason }}</strong><p class="mt-1 text-sm text-muted-foreground">{{ item.description }}</p><div v-if="canReview && item.status === 'pending'" class="mt-3 flex gap-2"><Button @click="reviewCorrection(item, 'resolve')">标记已处理</Button><Button variant="ghost" @click="reviewCorrection(item, 'reject')">驳回纠错</Button></div></li></ul></Card>
      </div>
    </template>
  </section>
</template>
