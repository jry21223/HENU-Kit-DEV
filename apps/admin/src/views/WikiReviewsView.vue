<template>
  <AdminShell>
    <div class="page-header">
      <div>
        <p class="eyebrow">Wiki Review</p>
        <h1>{{ copy.title }}</h1>
        <p class="muted">{{ copy.description }}</p>
      </div>
      <div class="action-row">
        <el-select v-model="statusFilter" class="status-filter" @change="loadEntries">
          <el-option v-for="item in statuses" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
        <el-button type="primary" :loading="loading" @click="loadEntries">{{ copy.refresh }}</el-button>
      </div>
    </div>
    ...
    <el-card class="section-card" shadow="never">
      ...
      <el-table v-loading="loading" :data="entries" empty-text="No wiki entries" style="width: 100%">
        <el-table-column :label="copy.entry" min-width="280">
          <template #default="{ row }">
            <strong>{{ row.title }}</strong>
            <p class="cell-muted">{{ row.slug }}</p>
            <p class="cell-muted">{{ copy.author }}: {{ row.authorId }}</p>
            <p class="cell-muted">{{ copy.course }}: {{ row.courseId || copy.noCourse }}</p>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="copy.version" width="100">
          ...
          <template #default="{ row }">
            <div class="table-actions">
              <el-button
                ...
                :data-testid="`wiki-review-approve-${row.id}`"
              ...
              <el-button
                ...
                plain
                :disabled="!canReview(row.status)"
                :data-testid="`wiki-review-reject-${row.id}`"
      ...
      <template #footer>
        <div class="action-row">
          ...
          <el-button
            ...
            :loading="reviewing"
            data-testid="wiki-review-submit"
            @click="submitReview"
          >
...
import { apiRequest, type WikiEntry } from "../lib/api";

const copy = {
  title: "\u0057\u0069\u006b\u0069 \u5ba1\u6838",
  description:
    "\u5ba1\u6838\u521b\u4f5c\u8005\u63d0\u4ea4\u7684\u8bfe\u7a0b\u5171\u521b\u8bcd\u6761\u3002\u901a\u8fc7\u540e\u624d\u4f1a\u51fa\u73b0\u5728\u516c\u5f00 Wiki \u5217\u8868\u3002",
  refresh: "\u5237\u65b0",
  queue: "\u5ba1\u6838\u961f\u5217",
  entry: "\u8bcd\u6761",
  author: "\u4f5c\u8005",
  ...
  notReviewed: "\u672a\u5ba1\u6838",
  reviewHint: "\u5ba1\u6838\u7ed3\u679c\u4f1a\u5199\u5165\u670d\u52a1\u7aef\u64cd\u4f5c\u65e5\u5fd7\u548c Wiki \u5ba1\u6838\u5b57\u6bb5\u3002",
  approvePlaceholder: "\u53ef\u9009\uff1a\u8bb0\u5f55\u901a\u8fc7\u4f9d\u636e\u6216\u540e\u7eed\u5907\u6ce8",
  rejectPlaceholder: "\u5fc5\u586b\uff1a\u8bf4\u660e\u9700\u8981\u4fee\u6539\u7684\u539f\u56e0",
  rejectReasonRequired: "\u8bf7\u586b\u5199\u9a73\u56de\u539f\u56e0\u3002",
  loadFailed: "Wiki \u5ba1\u6838\u961f\u5217\u52a0\u8f7d\u5931\u8d25",
  reviewDone: "Wiki \u5ba1\u6838\u72b6\u6001\u5df2\u66f4\u65b0\u3002",
  reviewFailed: "Wiki \u5ba1\u6838\u5931\u8d25",
  currentFilter: "\u5f53\u524d\u7b5b\u9009",
...
const reviewTarget = ref<WikiEntry | null>(null);
...
const metrics = computed(() => [
  ...
  { label: copy.currentResults, value: entries.value.length },
]);
...
async function loadEntries() {
  ...
  error.value = "";
  try {
    const response = await apiRequest<{ entries: WikiEntry[] }>(`/admin/wiki/entries?status=${statusFilter.value}`);
...
function openReview(entry: WikiEntry, action: "approve" | "reject") {
  reviewTarget.value = entry;
...
async function submitReview() {
  ...
  try {
    await apiRequest<{ reviewed: boolean; status: string; reviewReason: string }>(
      `/admin/wiki/entries/${reviewTarget.value.id}/${reviewAction.value}`,
