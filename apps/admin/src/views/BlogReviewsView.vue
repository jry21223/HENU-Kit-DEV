<template>
  <AdminShell>
    <div class="page-header">
      <div>
        <p class="eyebrow">Blog Review</p>
        <h1>{{ copy.title }}</h1>
        <p class="muted">{{ copy.description }}</p>
      </div>
      <div class="action-row">
        <el-select v-model="statusFilter" class="status-filter" @change="loadPosts">
          <el-option v-for="item in statuses" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
        <el-button type="primary" :loading="loading" @click="loadPosts">{{ copy.refresh }}</el-button>
      </div>
    </div>
    ...
    <el-card class="section-card" shadow="never">
      ...
      <el-table v-loading="loading" :data="posts" empty-text="No blog posts" style="width: 100%">
        <el-table-column :label="copy.post" min-width="260">
          ...
          <template #default="{ row }">
            <div class="table-actions">
              <el-button
                ...
                :data-testid="`blog-review-approve-${row.id}`"
              ...
              <el-button
                ...
                plain
                :disabled="!canReview(row.status)"
                :data-testid="`blog-review-reject-${row.id}`"
      ...
      <template #footer>
        <div class="action-row">
          ...
          <el-button
            ...
            :loading="reviewing"
            data-testid="blog-review-submit"
            @click="submitReview"
          >
...
import { apiRequest, type BlogPost } from "../lib/api";

const copy = {
  title: "\u535a\u5ba2\u5ba1\u6838",
  description:
    "\u5ba1\u6838\u5b66\u751f\u63d0\u4ea4\u7684\u5b66\u4e60\u7ecf\u9a8c\u548c\u590d\u4e60\u65b9\u6cd5\u535a\u5ba2\u3002\u901a\u8fc7\u540e\u624d\u4f1a\u51fa\u73b0\u5728\u516c\u5f00\u535a\u5ba2\u5217\u8868\u3002",
  refresh: "\u5237\u65b0",
  queue: "\u5ba1\u6838\u961f\u5217",
  post: "\u535a\u5ba2",
  author: "\u4f5c\u8005",
...
const reviewTarget = ref<BlogPost | null>(null);
...
const metrics = computed(() => [
  ...
  { label: copy.currentResults, value: posts.value.length },
]);
...
async function loadPosts() {
  ...
  error.value = "";
  try {
    const response = await apiRequest<{ posts: BlogPost[] }>(`/admin/blog/posts?status=${statusFilter.value}`);
...
function openReview(post: BlogPost, action: "approve" | "reject") {
...
async function submitReview() {
  ...
  try {
    await apiRequest<{ reviewed: boolean; status: string; reviewReason: string }>(
      `/admin/blog/posts/${reviewTarget.value.id}/${reviewAction.value}`,
