        ...
        >
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-[1fr_12rem]">
            <label className="block">
              <span className="font-mono text-[10px] tracking-[0.2em] text-ink/60">标题</span>
              <input
                required
                maxLength={160}
                value={title}
            ...
            <label className="block">
              <span className="font-mono text-[10px] tracking-[0.2em] text-ink/60">类别</span>
              <select
                value={category}
                onChange={(event) => {
                  resetCreateKeyOnEdit();
          ...
          <label className="mt-5 block">
            <span className="font-mono text-[10px] tracking-[0.2em] text-ink/60">问题说明</span>
            <textarea
              required
              maxLength={5000}
              value={body}
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <button
              ...
              className="inline-flex min-h-11 items-center justify-center border border-ink bg-ink px-4 py-2 font-mono text-xs tracking-widest text-paper transition-colors hover:border-accent hover:bg-accent hover:text-ink disabled:cursor-wait disabled:opacity-50"
            >
              ...
              tickets.map((ticket) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-2 font-mono text-[10px] tracking-[0.12em] text-ink/65">{ticket.reference}</p>
                    <p className="mt-2 font-mono text-[10px] text-ink/65">最后更新 {formatTimestamp(ticket.updated_at)}</p>
                  </button>
                );
              })
            )}
  ...
  return (
    <aside data-account-ticket-detail-state="success" className="border border-ink p-5 md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="font-mono text-[10px] tracking-[0.15em] text-ink/60">{ticket.reference}</p>
          <h2 className="mt-2 font-display text-2xl font-bold">{ticket.title}</h2>
        </div>
        <span className="border border-accent px-2 py-1 font-mono text-[10px] tracking-wider text-accent-text">
          {statusLabel(ticket.status)}
        </span>
      </div>
      <p className="mt-3 font-mono text-[10px] tracking-[0.1em] text-ink/60">版本 {ticket.version} · 更新于 {formatTimestamp(ticket.updated_at)}</p>
      ...
      {events.length > 0 ? (
        <ul className="mt-5 space-y-2 border-b border-line pb-5 font-mono text-[10px] leading-5 text-ink/60">
          {events.map((event) => (
            <li key={event.id}>
      ...
      <form onSubmit={onSubmitFollowUp} className="mt-6">
        <label className="block">
          <span className="font-mono text-[10px] tracking-[0.2em] text-ink/60">补充说明</span>
          <textarea
            required
            maxLength={5000}
            value={followUp}
        ...
        <button
          ...
          className="mt-4 inline-flex min-h-11 items-center justify-center border border-ink bg-ink px-4 py-2 font-mono text-xs tracking-widest text-paper transition-colors hover:border-accent hover:bg-accent hover:text-ink disabled:cursor-wait disabled:opacity-50"
        >
