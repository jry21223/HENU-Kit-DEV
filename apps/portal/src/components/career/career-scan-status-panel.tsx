  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm leading-6 text-ink/65">
        {emailEnabled
          ? "可以关闭本页面，扫描会在后台继续，完成后将发送结果简报至账户邮箱。"
...
function CompletedPanel({ search }: { search: CareerSearch }) {
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm leading-6 text-ink/75">扫描已完成，结果已保存。</p>
      <p className="mt-1 font-mono text-[10px] tracking-[0.15em] text-ink/40">
        #{search.id.slice(0, 8)} · {formatCareerSearchTime(search.created_at)}
        {search.has_email ? " · 结果简报已发送至账户邮箱" : " · 未发送邮件简报"}
      </p>
      <p className="mt-2 text-xs leading-5 text-ink/55">
        匹配岗位的查看功能将在后续版本开放，也可在历史页查看本次扫描记录。
      </p>
    </div>
  );
}
