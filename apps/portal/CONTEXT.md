...
- **One gesture, one screen**: A touch or pointer gesture steps at most one module section. Nothing is consumed before the finger lifts, so a gesture still in progress when a screen animation settles steps afterwards, and one that finds the first or last screen is not spent on the empty step.
