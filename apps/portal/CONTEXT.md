...
- **Wheel burst**: One continuous run of wheel ticks — a trackpad flick and its momentum tail, or one deliberate scroll. Wheel input has no release event and no readable velocity, so the run is grouped by time instead: a silence of 600ms or more starts a new gesture (comfortably longer than the 12–90ms spacing measured inside a flick, comfortably shorter than the ~1.2s a reader takes to wait out an animation and roll again), and the span counter of one window is capped at 3500ms (long enough to cover the 1.1s screen animation plus the tail still arriving after it, so a flick cannot start a second step); ticks swallowed by a running animation re-base that counter, so a window's wall-clock life is about one animation plus the cap.
