...
With `NEXT_PUBLIC_PORTAL_ENABLE_QUIZCRAFT_V2_READS=1`, `/practice/mistakes` reads the signed-in user's persistent learning-state facts only from Portal Gateway. The client always requests `wrong=true` and uses the contract's bounded `page` / `page_size` collection boundary (default `1` / `20`, maximum page size `100`); accessible previous/next controls consume the returned pagination so every filtered page remains reachable. The page renders stable bank/question/version references, wrong marks, counts, and timestamps; it never invents question content or falls back to browser-local or legacy data. Loading, signed-out, empty, owner-failure, retry, and refresh remain explicit UI states.
