...
- **Gesture intent**: What one touch or pointer gesture is asking for, judged once when the finger lifts, from that gesture's net displacement and peak velocity rather than from the movement accumulated so far. A step needs a net displacement of 6% of the viewport height, or 3% with a peak velocity of at least 600 px/s.
- **One gesture, one screen**: A touch or pointer gesture steps at most one module section. Nothing is consumed before the finger lifts, so a gesture that begins during a screen animation can still step once the animation settles, and one that finds the first or last screen is not spent on the empty step.
