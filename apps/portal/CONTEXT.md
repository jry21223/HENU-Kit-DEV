...
- **One gesture, one screen**: A touch, pointer, or wheel gesture steps at most one module section. Nothing is consumed before the finger lifts, so a gesture still in progress when a screen animation settles steps afterwards, and one that finds the first or last screen is not spent on the empty step.
- **Wheel burst**: One continuous run of wheel ticks — a trackpad flick and its momentum tail, or one deliberate scroll. Wheel input has no release event and no readable velocity, so the run is grouped by time instead: ticks more than 120ms apart are separate gestures, and a run is capped at 2000ms (comfortably past the 1.1s screen animation, so a tail still arriving when the animation ends cannot start a second step).
