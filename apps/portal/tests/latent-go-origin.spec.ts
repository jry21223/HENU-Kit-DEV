 ...
 * 边界上停得住——读者并不总停在边界上：视口变大（旋转屏幕、拉窗口）或内容回流会让
 * 整屏模块重新长高，浏览器保持 scrollY 不动，读者于是可能落到某个边界**上方**（不足
 * 一屏处）。
 *
 * 这类短落点里，δ 落在 35%~50% 视口高那一段时探针与视觉不一致：读者主要看着下面那
 * 一屏（恰好各占一半时按平局归它），35% 的探针却仍把他算在上一屏（δ ≤ 35% 时两者本
 * 来一致，δ > 50% 时他确实该算上一屏）。起点少算一屏的下场分两种：第一处边界上向上
 * 滑被判成「已经在第一屏」，`go(-1)` 算出 next === from 直接返回 false，整段手势被吃
 * 掉——屏幕一动不动，而且再滑几次也不会动，只能先向下滑一屏才解得开；更高的边界上
 * next ≠ from，手势不会空转，但读者会从自己看着的那一屏连退两屏。
 *
