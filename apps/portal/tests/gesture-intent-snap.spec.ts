 ...
 * 接管门槛（<768px、prefers-reduced-motion: reduce 退回普通滚动）与首末屏边界由
 * homepage-snap-scroll.spec.ts 守着，本文件不重复。
 */
...
/** 断言读者停在某一屏的顶端：比「翻到第几屏」更严——短落点/补跳都会在这里露馅。 */
...
test.describe("现状已具备、不许改坏的手势", () => {
  ...
  test("8px/帧 × 20 帧的慢速长拖动仍然切一屏", async ({ page }) => {
    ...
    // 累计 −160px、跨 2.4s：逐帧小位移靠桶累计照样构成一次意图（#507 已实测证伪
    // 「慢拖失效」）。收紧门槛之后它必须还能切屏，而且只切一屏。
    await swipeFinger(page, 640, 480, 20, 120);
