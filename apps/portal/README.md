...
- 收藏夹为空时给“去题库”；学习数据未开放或还没有记录、排行榜未开放或当前周期无人上榜时给“去刷题”；资料库与互助平台有搜索或筛选条件却没有结果时给“清除筛选”，恢复列表并把搜索框和筛选控件复位；按钮随空状态消失，焦点交给搜索与筛选区（不直接聚焦搜索框，免得手机弹出输入法）。没有任何条件的真实空列表不显示“清除筛选”。
