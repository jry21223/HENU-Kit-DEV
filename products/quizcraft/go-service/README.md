...
HC-18 and HC-19 add authenticated per-bank favorites and public rankings derived from immutable Practice Attempts. Ranking defaults to the current UTC Monday-to-Monday week and exposes only `rank`, a nullable internal `user_id`, and `correct_answer_count`; signed-in learners and guests (anonymous practice) compete under one identity key (`user_id` for signed-in learners, the immutable session `actor_key` for guests) and entries are capped at 100. Display-name synthesis is the Portal Gateway's responsibility: it resolves platform `users.display_name` (guests and unset names render `匿名学习者`) and MUST strip `user_id` before any external response. The ranking-profile write mechanism (PATCH `/api/v1/ranking-profile`) is removed; the `quizcraft_ranking_profiles` table is frozen and no longer read.
