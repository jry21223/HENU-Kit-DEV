...
type options struct {
	mode, targetURL, legacyURL, sourceName, runID string
...
func run(ctx context.Context, args []string) error {
	...
	value := options{}
	flags.StringVar(&value.mode, "mode", "full", "full, catch-up, or shadow-gate")
	flags.StringVar(&value.sourceName, "source-name", "quizcraft-legacy-production", "stable source identifier")
	flags.StringVar(&value.runID, "run-id", "", "full migration run UUID for catch-up")
	...
	value.targetURL = os.Getenv("DATABASE_URL")
	...
	case "full":
		...
		if err := quizcraft.RequireEmptyTarget(ctx, target); err != nil {
			return err
		}
		...
		report, err := service.RunFullMigration(ctx, snapshot)
		if err != nil {
			return err
		}
	...
	case "shadow-gate":
		...
		return nil
	default:
		return errors.New("mode must be full, catch-up, or shadow-gate")
	}
}
