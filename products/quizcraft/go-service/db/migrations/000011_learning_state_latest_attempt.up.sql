-- Versioned migrations run each source in one transaction. Match the rebuild
-- lock order so no submission can insert an immutable attempt between the
-- projection rebuild's fact scan and its replacement rows.
