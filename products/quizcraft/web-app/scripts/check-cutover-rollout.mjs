...
const ranking = fs.readFileSync(new URL('../src/pages/Ranking.tsx', import.meta.url), 'utf8');
const shadowClient = fs.readFileSync(new URL('../src/api/quizcraftShadowClient.ts', import.meta.url), 'utf8');
...
for (const forbidden of ['shadowRankingApi', 'RankingService', '/api/v1/rankings', '/api/v1/ranking-profile']) {
  if (ranking.includes(forbidden) || shadowClient.includes(forbidden)) {
    throw new Error(`standalone ranking invokes a V2 Core path before #166: ${forbidden}`);
  }
}
