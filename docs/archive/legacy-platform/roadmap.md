...
- Go API production preflight command for APP_ENV, CORS, auto-migrate, fixed verification code, JWT keys, WeChat Pay live config, mounted key/upload paths, and parseable non-expiring WeChat platform certificate checks
...
- Wiki creator application API, student-facing application/status UI, creator/admin entry submission page, user-facing Wiki submission tracking/resubmission, reviewer/admin approval flow, and user role promotion on approval
...
- Public all-time leaderboards for Wiki contributions, quiz practice, and overall learning score, plus Web `/leaderboards`
