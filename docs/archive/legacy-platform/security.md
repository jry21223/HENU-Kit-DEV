...
- Admin material upload, manual material creation with `storageKey`, and manifest import perform lightweight content checks before files become downloadable assets: PDFs must start with `%PDF`, text/Markdown files cannot contain NUL bytes, and DOCX files must have a ZIP header. Manual material creation derives `fileSize` from disk instead of trusting request metadata.
- Publishing a material through status update or review approval re-validates the referenced local file, so invalid or missing files cannot be made public by status changes alone.
- Runtime `uploads/` storage must remain private. Production Nginx returns 404 for `/uploads/`, the production Compose example mounts the upload volume only into API/Worker containers, and frontend code should use API media URLs instead of direct upload paths. Paid materials must always be fetched through `/api/v1/materials/:id/download`; moment images must be fetched through `/api/v1/moments/images/:id` so permission and visibility checks run.
...
- Payment incident list rows expose only an alert-history summary derived from `payment_incident.alert` operation logs: count, last alert time, and last alert operator id. They do not expose operation-log IP addresses, User-Agent values, or metadata.
