...
- 公开资料列表/详情、课程资料列表和课程包内资料使用脱敏 DTO，不暴露 storage key、创建人或审核元数据。
- 实现资料 manifest 导入基础版：
...
- 微信 Native 关单基础接口已接入：
  ...
  - live paying 订单会先调用微信关单接口，再更新本地状态；已补 `-wechat-live-native` smoke 用于目标环境验证，但真实目标环境仍需实际执行。
- 微信 Native 订单过期收敛已接入：
...
目前已有 mock Native codeUrl、前端二维码展示、开发/测试环境带 HMAC 的 mock notify 闭环、带响应验签的 live Native 下单代码路径、live order-and-close smoke，以及 live 官方 notify 验签、解密、金额校验和幂等授权代码路径。
...
- 会员积分兑换基础闭环已补；真实支付购买、升级/降级策略仍未完成。
- AI task 创建时的会员/积分扣减基础闭环已补；真实模型 token 计费和更细 AI 套餐仍未完成。
...
- AI 针对性出题完整闭环。
- AI 套卷生成完整闭环。
- AI 草稿发布到资料/Wiki/套卷的正式流程；题库 targeted_question 显式发布路径已补。
...
- 更丰富的互关好友 UX、筛选和批量关系管理。
- 更完整的用户主页资料编辑、勋章/成就和活动时间线。
- 搜索。
- 排行榜。
- 博客积分激励完整结算。
...
- Admin Material、Blog、Wiki entry、Wiki proposal、Forum post、Forum reply、Report handling 与 AI draft 审核已有 opt-in browser review smoke；真实 LLM/RAG、AI 发布流和更完整 AI E2E 仍需扩展。
- 移动端截图回归。
- Docker 全链路 smoke。
...
| Stage 4：组织架构与课程资料 | 组织/课程 CRUD、资料上传下载、水印、权限 | 部分完成 / 强 MVP | 组织、课程、资料、上传、下载权限、水印、审计、manifest dry-run 预检、内测发布门禁 `-check-release`、manifest 导入、manifest-to-paid-download 测试、API manual-grant smoke 和浏览器 delivery smoke 已实现；真实内测资料导入验收和 OSS/S3 等生产存储仍未完成。 |
...
| Stage 16：测试与质量 | 后端、前端、Docker、支付、审核等测试 | 部分完成 | Go tests、Web/Admin lint/build 已持续运行；已有 delivery、quiz wrong-question、admin material-review、admin blog-review、admin wiki-review、admin wiki-proposal-review、admin forum-review、admin forum-reply-review、admin report-review、admin ai-draft-review 和 mobile public-page browser smoke；缺少完整 E2E 和浏览器截图回归。 |
...
### 7.4 2026-06-24 Quiz Wrong-Question Smoke Update

- Web quiz submission now sends httpOnly auth cookies to the Go API through the shared `postApi` helper, so authenticated wrong answers can be persisted from the browser.
...
- The smoke mutates Blog rows, notifications, and operation logs. It covers the Blog review path only; broader moderation workflows still need more browser probes.

### 7.6 2026-06-24 Admin Material Review Smoke Update

- Public material list/detail, course material list, and public package material responses now use a redacted DTO that hides storage keys, creator ids, reviewer ids, reviewed timestamps, and review reasons.
- Vue Admin Material review actions now expose stable test ids for approve/reject and review submission buttons.
- `npm --workspace @final-review/web run test:e2e:material-review` adds an opt-in Playwright smoke that creates a pending Material row, verifies it is hidden from public detail before review, approves it through Vue Admin `/material-reviews`, and verifies the public API plus Web Material detail page render it after approval without leaking storage/review metadata.
- The smoke mutates Material rows, notifications, and operation logs. It does not download files; delivery/payment smoke still covers download permissions.

### 7.7 2026-06-24 Admin Wiki Review Smoke Update
...
- The smoke mutates Wiki rows, edit history, notifications, and operation logs. It covers the Wiki entry review path only; broader moderation workflows still need more browser probes.
...
- The smoke mutates Forum rows, notifications, and operation logs. It covers the Forum post review path only; broader moderation workflows still need more browser probes.
...
- The smoke mutates Forum post/reply rows, notifications, and operation logs. It covers the Forum reply review path only; broader moderation workflows still need more browser probes.
...
- The smoke mutates Wiki rows, edit history, notifications, and operation logs. It covers the happy-path proposal approval boundary; stale proposal conflict UI and broader moderation workflows still need more browser probes.

### 7.11 2026-06-24 Admin AI Draft Review Smoke Update

- Vue Admin AI draft review actions now expose stable test ids for approve/reject and review submission buttons.
- `npm --workspace @final-review/web run test:e2e:ai-draft-review` adds an opt-in Playwright smoke that creates a real AI task, waits for the worker to generate a pending AI draft, approves it through Vue Admin `/ai/drafts`, and verifies the reviewed draft has no `publishedId`.
- The smoke grants a temporary tier2 membership for AI quota, then mutates AI task/draft rows, AI usage logs, notifications, and operation logs. It covers the mock-worker draft review boundary only; real LLM/RAG, real model-token billing, broader publish-to-resource flows, and richer AI quality checks remain later work.

### 7.12 2026-06-24 Admin Report Review Smoke Update

- Vue Admin Report review actions now expose stable test ids for resolve/reject and review submission buttons.
- `npm --workspace @final-review/web run test:e2e:report-review` adds an opt-in Playwright smoke that creates and approves a public Blog target, submits a report from a separate reporter account, resolves it through Vue Admin `/reports`, verifies a `report_result` notification for the reporter, and confirms the target Blog content remains unchanged after report handling.
- The smoke mutates Blog, report, notification, and operation-log rows. It covers the happy-path report resolution boundary only; richer moderation actions such as hiding content, sanctions, appeal flow, and reviewer workload queues remain later work.
...
但当前仍不能声明为生产可上线版本。最大缺口是微信 Native 真实商户端到端联调、关单验证、退款/告警等支付运维硬化，其次是完整 E2E、真实 AI/RAG、真实模型成本核算、搜索增强和生产部署硬化。
...
## Addendum: 2026-06-24 Wiki Creator Application API

- Implemented logged-in normal-user creator applications through `POST /api/v1/wiki/creator-applications`.
- Implemented reviewer/admin list, approve, and reject APIs under `/api/v1/admin/wiki/creator-applications`.
