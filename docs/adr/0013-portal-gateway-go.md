...
- Portal 用户权限码与 Console 的 `console.*` 权限码完全隔离（Gateway 侧实际消费的权限码以各产品契约声明为准，如 `portal.practice.read`）。`portal.library.read`、`portal.food.read` 及对应 LIBRARY/FOOD owner OAuth client 已随 #261 移除，Gateway 侧无消费方。
...
- 2026-08-07（#261）：移除废弃的 LIBRARY/FOOD owner OAuth client 及其在 Gateway 侧的使用；`portal.library.read`、`portal.food.read` 权限码随 owner client 删除，上述「决策」条目已更新为现状描述。
