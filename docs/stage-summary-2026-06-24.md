...
- 完整 E2E、移动端截图回归、浏览器自动化验收；当前已有 delivery、quiz wrong-question、admin blog-review 和 mobile public-page 多条 opt-in/target smoke，但还不是完整回归套件。
...
| Stage 16：测试与质量 | 后端、前端、Docker、支付、审核等测试 | 部分完成 | Go tests 和部分 build/lint 已持续运行；已补 delivery、quiz wrong-question、admin blog-review、mobile public-page browser smoke；仍缺完整 E2E 和截图回归。 |
...
- 缺少完整 E2E 和移动端截图回归，界面体验风险仍需单独验收；quiz wrong-question 和 admin blog-review smoke 只能覆盖关键单路径，不等于完整刷题/审核回归。
...
4. 继续补 E2E smoke：当前已有 delivery、quiz wrong-question、admin blog-review 和 mobile public-page；下一步补资料/Wiki/论坛审核、订单支付回调模拟和更多刷题题型。
