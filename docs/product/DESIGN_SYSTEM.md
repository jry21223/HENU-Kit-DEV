...
- 强调橙 `#FF4D00` 用于色块和品牌标志（字标没有对比度要求，字标里的 ® 保持强调橙）；作文字只用在墨色底上（在 `#161513` 上 5.49:1）。它在纸白上只有 2.92:1，在 `--hk-accent-soft` 上 2.85:1，连大字的 3:1 也不到。
- 浅色底上的橙字不论大小（编号、眉标、链接、大号数字，以及项目符号「+」、十字对位标记、加载中的「…」这类橙色符号）都用 `--hk-accent-text`：纸白上 5.01:1，白色卡片上 5.71:1，`--hk-accent-soft` 上 4.88:1，5% 强调橙叠在纸白上（Portal 的提示框与选中态 `bg-accent/5`）4.73:1，5% 墨色叠在纸白上（Portal 列表行悬停 `hover:bg-ink/5`）4.53:1。它是这类文字的语义基线，看起来仍是强调橙，只是更深。`--hk-accent-deep` 在纸白上只有 3.90:1，不当文字色。
- 强调橙色块上的文字用墨色（5.49:1），不用纸白（2.92:1），例如首页跑马灯和选中文字的高亮。悬停时转成强调橙底的按钮，文字同时转成墨色。
- 次要说明用 `--hk-ink-muted`（纸白上 4.75:1）。
- Portal 用墨色透明度写灰字（`text-ink/NN`），下限 `ink/60`（纸白上 4.56:1，白色卡片上 4.77:1）。叠在 5% 色块上时下限 `ink/65`（5.16:1 起），`ink/60` 在这些底上只有 4.4:1：列表行悬停或选中（`bg-ink/5`）、提示框与选中态（`bg-accent/5`）、压在墨色模块上的首页页头（95% 纸白）。占位文字同样按这条线。
- 墨色底上的纸白字下限 `paper/50`（4.78:1）。
- 低于这条线的灰色只用于纯装饰，并且整块加 `aria-hidden`：十字对位标记、分隔符、箭头、卡片与占位图块上的类型代号和占位大字、骨架卡片里的「--」。说明文字、状态、计数、序号和标签都算信息，照上面的下限配色，不能靠 `aria-hidden` 绕过；其中的大字按大字的 3:1 配色，例如首页美食榜 36px 粗体的名次用 `ink/50`（3.35:1）。没有图的互助卡片以分类代号作占位大字，读屏另读分类名。
- 禁用的控件没有对比度要求，但要看得出不可用：颜色变淡，配合 `cursor-not-allowed` 或 `cursor-wait`。

以上配色由 `apps/portal/src/app/design-tokens.test.ts` 按 `tokens.json` 计算检查。页面上的实际对比度由 `apps/portal/tests/color-contrast.spec.ts` 检查：axe 的 `color-contrast` 规则在首页（逐屏滚动）、五个子站首页和登录页上，1440 与 390 两种宽度都为 0；几类按钮和链接的悬停态另作抽查。这个 spec 跑在默认构建上，题库目录关闭，/practice 只有空状态；生产构建开着目录，题库卡片和目录加载失败的提示由 `apps/portal/tests/quizcraft-catalog.spec.ts` 在目录开启的 dev server 上同样检查。检查逻辑在 `apps/portal/tests/support/color-contrast.ts`。
...
| Meta | 13–14px | 13–14px | 标签和辅助信息；最小 12px，见下方“字号下限” |

操作页降低标题张力，优先扫描效率。正文行宽建议不超过 72 个中文字符。等宽编号标签（如 `01 / 02 / 03`、`A-01`）是主站的工程图纸风格标记，仅用于学生端页面。

### 字号下限

- 看得见的文字不小于 12px（Tailwind `text-xs`）：编号、眉标、计数、日期、状态角标、表单标签和提示都算。
- 小于 12px 的字只用于加了 `aria-hidden` 的纯装饰拉丁标签，最小 10px，例如资料卡封面上的类型代号、十字对位标记、首屏眉标旁旋转的 ®。装饰里的中文同样不小于 12px。
- SVG 里的字随 viewBox 缩放，按屏幕上的大小算：图变窄时放大字号（首页互助流程图的节点文字），装饰性的刻度和编号在图太窄时不显示（求职雷达的角度刻度）。

### 字距

- 宽字距（`tracking-wide` 及更宽，或正值的 `tracking-[…em]`）只加在拉丁字母、数字和等宽编号上，这是编号与英文眉标的图纸风格。
- 中文使用 `tracking-normal`。中文本身是等宽方块，再拉开字距会读成一个字一个字的碎片（“学 校 邮 箱”）。
- 中英混排的标签拆开处理：拉丁部分放进单独的 span 加字距，中文部分不加。例如 `<span className="tracking-widest">01</span>资料库`、`<span className="tracking-[0.3em]">SCROLL</span> / 向下滚动查看模块`。标签来自数据时同理，只给拉丁条目加字距（首页跑马灯）。

以上规则由 `apps/portal/src/app/typography.test.ts` 按源码检查：写死的小于 12px 的字号类只能出现在读屏隐藏的拉丁装饰上，加了字距的元素里不能直接写中文。页面上的实际效果由 `apps/portal/tests/typography.spec.ts` 按计算样式检查：首页、五个子站首页和登录页在 1440 与 390 两种宽度下，看得见的文字都不小于 12px（读屏隐藏的拉丁装饰不小于 10px），含中文的文字字距都不超过 0.05em；题库目录开启时的 /practice 由 `apps/portal/tests/quizcraft-catalog.spec.ts` 同样检查。检查逻辑在 `apps/portal/tests/support/typography.ts`。
...
- Primary：墨色（`--hk-ink`）背景、纸白文字；Hover 转为强调橙底、墨色文字（与主站 `button.tsx` 一致）。
