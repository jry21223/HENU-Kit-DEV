...
- the payment incident list response includes `incidents` and `total`; incident rows are DTOs for triage and deliberately omit `rawNotify` and `idempotencyKey`. They include `alertCount`, `lastAlertedAt`, and `lastAlertedBy` derived from `payment_incident.alert` operation logs without exposing log IP, user-agent, or metadata. Vue Admin uses the summary endpoint to surface open payment incident alerts and high-risk distribution on the dashboard
