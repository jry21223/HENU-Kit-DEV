...
- WeChat Pay live cryptographic helpers are implemented as tested pure functions: RSA request signing, Authorization header construction, notify signature verification against platform public keys, merchant private-key parsing, and AES-256-GCM resource decryption. They are not yet wired into live payment HTTP handlers.
