...
单人维护时同一人可以承担实现和最终验收，但实现、测试、双轴审查、发布批准与上线观察必须分步骤执行并绑定当前 SHA。资源不足时，优先延后目录移动、排行榜切流和可选功能，不降低账号、数据和回滚门槛。
...
| 角色 | 主责 | 必须留存的独立证据 |
|---|---|---|
| 开发 A | Platform Core、OpenAPI、身份、权限、事件、数据迁移 | 当前 SHA 的测试、双轴审查、Migration 往返与回滚结果 |
| 开发 B | Portal、Study/Quiz 接入、Worker、CI/CD、目录迁移 | 当前 SHA 的构建、E2E、部署切换与回滚结果 |
| 测试 A | API 契约、数据库、并发、幂等、安全、数据一致性 | 可复现的命令、结果和数据对账 |
| 测试 B（可选） | 浏览器 E2E、移动端、邮件送达、灰度、改名和恢复演练 | 浏览器矩阵、送达、灰度和恢复记录 |
