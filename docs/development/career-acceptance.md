...
- 标签页刷新继续保留当前任务：同一任务不重复恢复；从历史深链发起新扫描后，
  焦点刷新不会让旧任务抢回面板。回归见 `refocusing the tab does not re-restore
  an unchanged scan` 与 `a new scan keeps ownership after refocusing a historical
  deep link`。
