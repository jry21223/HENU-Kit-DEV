...
| ID | 场景 | 预期 |
|---|---|---|
| NOTICE-001 | 表单与 JSONL 输入同一通知 | Upsert 结果和版本语义一致 |
| NOTICE-002 | JSONL 超过 1,000 条或 10 MB | 拒绝超限部分并返回可定位错误 |
| NOTICE-003 | 相同内容重复导入 | 幂等成功，不创建新版本 |
| NOTICE-004 | 内容 Hash 变化 | 新建不可变版本，旧版本保留 |
| NOTICE-005 | JSONL 缺少 Hash 或 Schema 错误 | 逐行失败，不污染已成功行 |
...
| NOTICE-012 | 附件预签名上传 | MIME/大小受限，浏览器不接触 S3 Secret |
...
| PF-004 | urgent 到期时间 | 创建后 24 小时到期 |
| PF-005 | normal 到期时间 | 创建后 72 小时到期 |
| PF-006 | 超时计算 | 未解决且超过 due_at 才计入 |
| PF-007 | 受限管理员调整 due_at | 权限受限并写 before/after 审计 |
...
| FOOD-008 | 有效参与者少于 10 | `insufficient_votes`，不能调档 |
| FOOD-008A | 任一方向达到 70% | 形成升档、降档或稳定结论 |
| FOOD-008B | 最近调档不足 7 天 | `cooldown`，不能再次调档 |
...
| OPS-003 | 审计查询越权 | 拒绝访问 |
| OPS-004 | 高风险操作 | actor、resource、before/after、request_id、IP 前缀完整 |
