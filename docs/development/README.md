...
本实施索引不另行复制一套优先级。发生冲突时遵循[文档中心的规范层级](../README.md#4-文档优先级)和仓库级开发规范的“文档优先级”章节；安全、隐私、法律要求与 Accepted ADR 不能被本包、应用 README、旧计划或现有实现覆盖。

代码或运行证据与决策文档不一致时，记录真实状态并创建差异 Issue，不把现有实现自动视为决策更新。
