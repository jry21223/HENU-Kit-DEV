...
- Admin Material、Blog、Wiki entry、Wiki proposal、Forum post、Forum reply 与 AI draft 审核已有 opt-in browser review smoke；真实 LLM/RAG、AI 发布流和更完整 AI E2E 仍需扩展。
- 移动端截图回归。
- Docker 全链路 smoke。
...
| Stage 16：测试与质量 | 后端、前端、Docker、支付、审核等测试 | 部分完成 | Go tests、Web/Admin lint/build 已持续运行；已有 delivery、quiz wrong-question、admin material-review、admin blog-review、admin wiki-review、admin wiki-proposal-review、admin forum-review、admin forum-reply-review、admin ai-draft-review 和 mobile public-page browser smoke；缺少完整 E2E 和浏览器截图回归。 |
...
- The smoke mutates Blog rows, notifications, and operation logs. It covers the Blog review path only; report handling still needs its own browser probe.
...
- The smoke mutates Wiki rows, edit history, notifications, and operation logs. It covers the Wiki entry review path only; report handling still needs its own browser probe.
...
- The smoke mutates Forum rows, notifications, and operation logs. It covers the Forum post review path only; report handling still needs its own browser probe.
...
- The smoke mutates Forum post/reply rows, notifications, and operation logs. It covers the Forum reply review path only; report handling still needs its own browser probe.
...
- The smoke mutates Wiki rows, edit history, notifications, and operation logs. It covers the happy-path proposal approval boundary; stale proposal conflict UI and report handling still need broader browser probes.
