...
- Payment incident routing is still basic: incidents can be recorded, summarized on the admin dashboard with an overdue threshold, handled manually, manually re-alerted through the signed webhook, and checked with the read-only `-payment-ops-readiness` smoke, but there is no full operations escalation workflow yet.
