...
**prebuilt 契约 `:?` 强制变量静态清单**（来自仓库 `docker-compose.henukit.prebuilt.yml`，共 60 个去重键；若服务器有该文件则直接跑 §2.4 脚本，否则人工对照本清单与 §2.3 键名清单）：

`RELEASE_SHA, PORTAL_SUMMARY_CLIENT_SECRET, PORTAL_VERSION, PORTAL_DEPLOYED_AT, PLATFORM_CORE_DATABASE_URL, PLATFORM_CORE_REDIS_URL, PLATFORM_CORE_IDEMPOTENCY_KEY, PLATFORM_CORE_VERIFICATION_KEY, PLATFORM_CORE_MAIL_DELIVERY_TOKEN, PLATFORM_CORE_MAIL_PROVIDER_TOKEN, PLATFORM_CORE_SMTP_ADDRESS, PLATFORM_CORE_SMTP_USERNAME, PLATFORM_CORE_SMTP_PASSWORD, PLATFORM_CORE_SMTP_FROM, STUDY_DATABASE_URL, ACCOUNT_PORTFOLIO_DATABASE_URL, ACCOUNT_PORTFOLIO_CLIENT_ID, ACCOUNT_PORTFOLIO_KEY_ID, ACCOUNT_PORTFOLIO_CLIENT_SECRET, ACCOUNT_PORTFOLIO_CONSOLE_CLIENT_ID, ACCOUNT_PORTFOLIO_CONSOLE_KEY_ID, ACCOUNT_PORTFOLIO_CONSOLE_SECRET, ACCOUNT_PORTFOLIO_POINT_CURSOR_KEY, NOTICE_DATABASE_URL, NOTICE_REDIS_URL, NOTICE_SUMMARY_CLIENT_ID, NOTICE_SUMMARY_KEY_ID, NOTICE_SUMMARY_CLIENT_SECRET, FOOD_DATABASE_URL, FOOD_REDIS_URL, FOOD_SUMMARY_CLIENT_ID, FOOD_SUMMARY_KEY_ID, FOOD_SUMMARY_CLIENT_SECRET, CAREER_CLIENT_SECRET, CAREER_DATABASE_URL, LIBRARY_DATABASE_URL, LIBRARY_REDIS_URL, LIBRARY_SUMMARY_CLIENT_ID, LIBRARY_SUMMARY_KEY_ID, LIBRARY_SUMMARY_CLIENT_SECRET, LIBRARY_DOWNLOAD_CLIENT_ID, LIBRARY_DOWNLOAD_KEY_ID, LIBRARY_DOWNLOAD_CLIENT_SECRET, LIBRARY_OSS_BUCKET, LIBRARY_OSS_REGION, LIBRARY_OSS_INTERNAL_ENDPOINT, LIBRARY_OSS_PUBLIC_ENDPOINT, LIBRARY_OSS_ECS_RAM_ROLE, STUDY_LEGACY_API_URL, STUDY_LEGACY_ADMIN_TOKEN, PLATFORM_CLIENT_SECRET, PORTAL_SESSION_KEY, LIBRARY_DOWNLOAD_URL, QUIZCRAFT_CORE_URL, QUIZCRAFT_PORTAL_CATALOG_CLIENT_ID, QUIZCRAFT_PORTAL_CATALOG_CLIENT_SECRET, QUIZCRAFT_PORTAL_CATALOG_KEY_ID, PRACTICE_COMMAND_CLIENT_ID, PRACTICE_COMMAND_CLIENT_SECRET, PRACTICE_COMMAND_KEY_ID, NOTICE_CLIENT_SECRET, ACCOUNT_PORTFOLIO_CLIENT_SECRET, CONSOLE_PLATFORM_CLIENT_SECRET, CONSOLE_SESSION_KEY, PLATFORM_SUMMARY_CLIENT_SECRET, NOTICE_SUMMARY_CLIENT_SECRET, LIBRARY_SUMMARY_CLIENT_SECRET, QUIZCRAFT_SUMMARY_CLIENT_SECRET, FOOD_SUMMARY_CLIENT_SECRET, POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_DB`
...
| 2 | `/opt/henukit/.env.henukit` 是否设置 `STUDY_LEGACY_API_URL`/`STUDY_LEGACY_ADMIN_TOKEN`（决定 library 能否启动） | §2 | 仓库 `.env.henukit.prod`/`.env.henukit` 均无此二键；library 按仓库配置 fail-closed 无法启动 |
