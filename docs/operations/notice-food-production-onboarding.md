...
- Console 网关通过 `NOTICE_API_URL` / `FOOD_API_URL` 指向对应服务；变量未定义时使用 Compose 私网默认值，显式置空时优雅降级为不可用且不会 panic
...
| 变量 | 说明 | 默认 |
|---|---|---|
| `NOTICE_API_URL` | console-gateway 访问 notice 的基地址；显式置空则模块降级为不可用 | `http://notice:8094` |
| `FOOD_API_URL` | console-gateway 访问 food 的基地址；显式置空则模块降级为不可用 | `http://food:8096` |
...
3. 激活发布时设置上表环境变量；`NOTICE_DATABASE_URL` / `FOOD_DATABASE_URL` / Redis / 服务间凭据缺失时 compose 直接报错（`:-?`），避免「以为在跑其实没接上」的半配置状态。这与其余服务的强制校验一致。`NOTICE_API_URL` / `FOOD_API_URL` 未定义时自动连接 Compose 私网 owner，显式置空时网关将对应模块降级为不可用；两个 `*_SUMMARY_URL` 仍需单独配置，未配置时仅概览摘要降级。
...
- 显式置空 `NOTICE_API_URL` / `FOOD_API_URL` 时：网关对对应模块返回 503 降级，不 panic（网关既有降级逻辑，未改动）。
