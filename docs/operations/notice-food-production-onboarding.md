...
- Notice 固定区分两名签名调用方：Console 管理调用由 `NOTICE_SERVICE_*` 校验，并与 console-gateway 的 `NOTICE_SUMMARY_*` 一一对应；Portal 只读通知流由 `NOTICE_PORTAL_*` 校验，并与 portal-gateway 的 `NOTICE_CLIENT_*` 一一对应。两对凭据必须分别生成，不得复用。
- Food 只接受 console-gateway 的签名调用，校验 `FOOD_SUMMARY_*` 凭据对（Gateway 侧展示的同一对凭据）
...
| `NOTICE_SERVICE_CLIENT_ID/SECRET/KEY_ID` | notice 的 Console 管理调用校验凭据 | 与 console-gateway 的 `NOTICE_SUMMARY_*` 一一对应；不得与 Portal 对复用 |
| `NOTICE_PORTAL_CLIENT_ID/SECRET/KEY_ID` | notice 的 Portal 只读通知流校验凭据 | 与 portal-gateway 的 `NOTICE_CLIENT_*` 一一对应；不得与 Console 对复用 |
...
3. 激活发布时设置上表环境变量；`NOTICE_DATABASE_URL` / `FOOD_DATABASE_URL` / Redis / 服务间凭据（包括彼此独立的 `NOTICE_SERVICE_*` 与 `NOTICE_PORTAL_*`）缺失时 compose 直接报错（`:-?`），避免「以为在跑其实没接上」的半配置状态。这与其余服务的强制校验一致。`NOTICE_API_URL` / `FOOD_API_URL` / 两个 `*_SUMMARY_URL` 未配置或置空时保持空值（与 `LIBRARY_API_URL` 同模式），网关照旧降级为不可用。
