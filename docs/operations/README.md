...
- [`henukit-artifact-deployment.md`](./henukit-artifact-deployment.md)：**生产部署契约** — GitHub Actions 编译固定 SHA 镜像 → 下载校验 → 主机 `docker load` + `deploy-henukit-artifact.sh`（主机禁止源码编译）。
- [`github-webhook-deploy.md`](./github-webhook-deploy.md)：Monorepo GitHub Webhook 自动同步、精确 SHA 发布、Systemd/Nginx、批准与回滚 Runbook（与成品镜像契约配合，不替代 artifact 部署）。
