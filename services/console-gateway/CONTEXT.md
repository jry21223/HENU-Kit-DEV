...
HC-09 establishes the login, callback, logout, and server-verified Console Access Context flow. HC-10 and HC-11 add the six-owner Overview aggregation and first real Portal owner endpoint. HC-12 adds thin signed forwarding for Platform Operations reads, idempotent Session/access writes, and unknown-result lookup. HC-13 adds the same thin boundary for Notice. HC-14 adds signed forwarding to the bounded Library Compatibility Adapter; Gateway verifies `library.*` against product Scope `library`, owns no Library state, and has no Study database credential. Browser responses never contain the exchange token or service credentials.
