...
func runTests(m *testing.M) int {
	if os.Getenv("FOOD_TEST_DATABASE_URL") != "" && os.Getenv("FOOD_TEST_REDIS_ADDR") != "" {
		return m.Run()
	}
	ctx := context.Background()
	postgres, err := testcontainers.Run(ctx, "postgres:16-alpine",
		testcontainers.WithEnv(map[string]string{"POSTGRES_DB": "food_test", "POSTGRES_USER": "food", "POSTGRES_PASSWORD": "food"}),
		testcontainers.WithExposedPorts("5432/tcp"),
		testcontainers.WithWaitStrategy(wait.ForLog("database system is ready to accept connections").WithOccurrence(2).WithStartupTimeout(60*time.Second)),
	)
	if err != nil {
		panic(fmt.Errorf("start Food PostgreSQL: %w", err))
	}
	...
	if err != nil {
		panic(fmt.Errorf("start Food Redis: %w", err))
	}
	...
	databaseURL := fmt.Sprintf("postgres://food:food@%s:%s/food_test?sslmode=disable", host, port.Port())
	...
	_ = os.Setenv("FOOD_TEST_DATABASE_URL", databaseURL)
	_ = os.Setenv("FOOD_TEST_REDIS_ADDR", redisHost+":"+redisPort.Port())
	return m.Run()
}
