...
`known_hosts`, into one root-owned non-symlink staging directory with a fully
root-trusted ancestor chain. Install GitHub CLI from the OS package as the
root-owned `/usr/bin/gh`; a user-owned `gh` binary is not a trust root.
