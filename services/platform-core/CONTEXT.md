...
The Account Center Bootstrap is a bounded browser contract for Portal-owned account presentation. It returns only the requested account flow and a CSRF token, sets the signed device/CSRF cookies, and exposes no credentials, Session token, OAuth state, PKCE challenge, Authorization Code, or redirect URI. Direct `GET` requests for login, registration, recovery, or account-security presentation redirect to the Portal Account Center. Credential form posts return only empty success statuses or bounded JSON errors and never render Platform Core HTML. Platform Core remains the sole credential and Core Session authority.
