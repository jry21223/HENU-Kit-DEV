...
func resetIdentityTables(t *testing.T, ctx context.Context, pool *pgxpool.Pool, redisClient *redis.Client) {
	t.Helper()
	if _, err := pool.Exec(ctx, `ALTER TABLE mail_outbox_audit_events DISABLE TRIGGER mail_outbox_audit_events_immutable; ALTER TABLE operations_inbox_audit_events DISABLE TRIGGER operations_inbox_audit_events_immutable; ALTER TABLE platform_operations_audit_events DISABLE TRIGGER platform_operations_audit_events_immutable; ALTER TABLE operator_bootstrap_audit_events DISABLE TRIGGER operator_bootstrap_audit_events_immutable; TRUNCATE operator_bootstrap_audit_events, platform_operations_audit_events, platform_operations_idempotency, operations_inbox_audit_events, operations_inbox_idempotency, operations_inbox_items, mail_outbox_audit_events, mail_delivery_receipts, mail_outbox, verification_codes, permission_codes, authorization_roles, authorization_codes, sessions, oauth_clients, users CASCADE; ALTER TABLE operator_bootstrap_audit_events ENABLE TRIGGER operator_bootstrap_audit_events_immutable; ALTER TABLE platform_operations_audit_events ENABLE TRIGGER platform_operations_audit_events_immutable; ALTER TABLE operations_inbox_audit_events ENABLE TRIGGER operations_inbox_audit_events_immutable; ALTER TABLE mail_outbox_audit_events ENABLE TRIGGER mail_outbox_audit_events_immutable`); err != nil {
