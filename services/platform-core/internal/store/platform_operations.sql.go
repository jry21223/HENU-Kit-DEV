...
const listConsoleUserIdentities = `-- name: ListConsoleUserIdentities :many
SELECT users.id, users.display_name, email_identities.email_ciphertext, users.status
FROM users
...
type ListConsoleUserIdentitiesRow struct {
	ID              pgtype.UUID `json:"id"`
	DisplayName     pgtype.Text `json:"display_name"`
	EmailCiphertext []byte      `json:"email_ciphertext"`
	Status          string      `json:"status"`
...
func (q *Queries) ListConsoleUserIdentities(ctx context.Context, dollar_1 []pgtype.UUID) ([]ListConsoleUserIdentitiesRow, error) {
	rows, err := q.db.Query(ctx, listConsoleUserIdentities, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListConsoleUserIdentitiesRow{}
	for rows.Next() {
		var i ListConsoleUserIdentitiesRow
		if err := rows.Scan(
			&i.ID,
			&i.DisplayName,
			&i.EmailCiphertext,
...
const listPlatformOperationMembershipAccounts = `-- name: ListPlatformOperationMembershipAccounts :many
SELECT users.id, users.display_name, users.status, identities.email_ciphertext
FROM users
JOIN email_identities AS identities ON identities.user_id = users.id
WHERE $1::text = ''
   OR strpos(lower(coalesce(users.display_name, '')), lower($1::text)) > 0
   OR ($2::bytea IS NOT NULL
       AND identities.email_lookup_hash = $2::bytea)
ORDER BY users.created_at DESC, users.id
LIMIT $4 OFFSET $3
`

type ListPlatformOperationMembershipAccountsParams struct {
...
type ListPlatformOperationMembershipAccountsRow struct {
	ID              pgtype.UUID `json:"id"`
	DisplayName     pgtype.Text `json:"display_name"`
	Status          string      `json:"status"`
	EmailCiphertext []byte      `json:"email_ciphertext"`
...
func (q *Queries) ListPlatformOperationMembershipAccounts(ctx context.Context, arg ListPlatformOperationMembershipAccountsParams) ([]ListPlatformOperationMembershipAccountsRow, error) {
	rows, err := q.db.Query(ctx, listPlatformOperationMembershipAccounts,
		arg.Search,
		arg.EmailLookupHash,
		arg.PageOffset,
		arg.PageLimit,
	...
	defer rows.Close()
	items := []ListPlatformOperationMembershipAccountsRow{}
	for rows.Next() {
		var i ListPlatformOperationMembershipAccountsRow
		if err := rows.Scan(
			&i.ID,
			&i.DisplayName,
			&i.Status,
