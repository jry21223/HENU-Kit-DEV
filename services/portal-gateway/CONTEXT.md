...
`GET /api/v1/learning-state` is a separate caller-owned, contract-paginated read. Portal Gateway validates `page` / `page_size` and the optional boolean `wrong` filter before either owner call, decodes its own Portal Session, asks Platform Core for `portal.practice.read` scoped to QuizCraft, requires Core's authorization actor to equal the session subject, and only then signs that actor and the exact query to QuizCraft's internal learning-state route. The same normalized request ID is sent to both owners, and a QuizCraft success is accepted only when its response header and body repeat that ID and its items satisfy any requested `wrong` value. The QuizCraft client neither uses environment proxies nor follows redirects. Signed-out, denied, mismatched, unavailable, or invalid-owner responses fail closed without a legacy, mock, or browser-to-Core fallback.
