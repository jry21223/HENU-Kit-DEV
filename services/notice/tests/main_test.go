...
func runTests(m *testing.M) int {
	if os.Getenv("NOTICE_TEST_DATABASE_URL") != "" {
		return m.Run()
	}
	ctx := context.Background()
	postgres, err := testcontainers.Run(ctx, "postgres:17-alpine",
		testcontainers.WithEnv(map[string]string{"POSTGRES_DB": "notice_test", "POSTGRES_USER": "notice", "POSTGRES_PASSWORD": "notice"}),
		testcontainers.WithExposedPorts("5432/tcp"),
		testcontainers.WithWaitStrategy(wait.ForLog("database system is ready to accept connections").WithOccurrence(2).WithStartupTimeout(60*time.Second)),
	)
	if err != nil {
		panic(fmt.Errorf("start notice PostgreSQL: %w", err))
	}
	...
	databaseURL := fmt.Sprintf("postgres://notice:notice@%s:%s/notice_test?sslmode=disable", host, port.Port())
	...
	_ = os.Setenv("NOTICE_TEST_DATABASE_URL", databaseURL)
	return m.Run()
}
