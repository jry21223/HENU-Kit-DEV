...
- Vue Admin includes `/payment-incidents` for admin-only WeChat callback anomaly triage, and `/dashboard` surfaces the current open-incident count. Resolving or ignoring an incident records an operation note only; it does not mark orders paid or grant entitlement.
