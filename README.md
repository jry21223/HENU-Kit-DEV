...
- WeChat paid-order verification smoke: `go run ./cmd/smoke -verify-paid-order -order-id <paid-order-id> -email <buyer-email> -code <code>` after a real internal payment. It only verifies backend state: order `paid`, entitlement granted, matching package detail, and paid material download.
- Payment ops readiness smoke: `go run ./cmd/smoke -skip-login -admin-email <admin-email> -payment-ops-readiness` is a read-only admin gate that fails on critical/high reconciliation issues or critical/high/overdue open payment incidents. It does not create orders, call WeChat, grant entitlement, or resolve incidents.
...
- Admin wiki-proposal-review smoke: `npm --workspace @final-review/web run test:e2e:wiki-proposal-review` with `E2E_WIKI_PROPOSAL_REVIEW_SMOKE=1` creates a published Wiki baseline, submits an edit proposal, verifies public content stays unchanged before review, approves it through Vue Admin, and verifies public API/Web content updates only after review.
...
- Admin forum-reply-review smoke: `npm --workspace @final-review/web run test:e2e:forum-reply-review` with `E2E_FORUM_REPLY_REVIEW_SMOKE=1` creates a pending Forum reply under an approved smoke post, approves it through Vue Admin, and verifies public API/Web visibility only after review.
- Admin report-review smoke: `npm --workspace @final-review/web run test:e2e:report-review` with `E2E_REPORT_REVIEW_SMOKE=1` creates a public Blog target, submits a report, resolves it through Vue Admin, verifies reporter notification, and confirms report handling does not rewrite the target content.
- Admin AI draft-review smoke: `npm --workspace @final-review/web run test:e2e:ai-draft-review` with `E2E_AI_DRAFT_REVIEW_SMOKE=1` creates an AI task, waits for the worker to generate a draft, approves it through Vue Admin, and verifies review does not auto-publish generated content.
...
- Vue Admin includes `/payment-incidents` for admin-only WeChat callback anomaly triage, and `/dashboard` uses the read-only incident summary endpoint to surface open count, overdue count, high-risk count, and top incident type. `PAYMENT_INCIDENT_OVERDUE_MINUTES` controls the dashboard overdue threshold. New incidents can optionally post a signed best-effort webhook through `PAYMENT_INCIDENT_WEBHOOK_URL`; admins can manually re-alert open incidents after checking the incident page. Production preflight requires configured webhooks to use HTTPS and `PAYMENT_INCIDENT_WEBHOOK_SECRET`. Re-alerting, resolving, or ignoring an incident records an operation note only; it does not mark orders paid or grant entitlement.
