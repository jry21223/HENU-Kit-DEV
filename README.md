...
- Vue Admin includes `/payment-incidents` for admin-only WeChat callback anomaly triage, and `/dashboard` uses the read-only incident summary endpoint to surface open count, high-risk count, and top incident type. New incidents can optionally post a signed best-effort webhook through `PAYMENT_INCIDENT_WEBHOOK_URL`. Resolving or ignoring an incident records an operation note only; it does not mark orders paid or grant entitlement.
