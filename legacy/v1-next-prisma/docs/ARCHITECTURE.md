...
当前目标支付路径为微信支付 Native（`wechat_native`）。文档中已有 EasyPay / 易支付内容均属于历史遗留描述，不能作为后续默认实现方向。核心课程资料以人工准备、manifest/后台导入、课程包绑定、服务端 entitlement 鉴权交付为主，AI 生成资料不是当前主流程。
...
- 支付方向已调整为微信支付 Native；旧 EasyPay 兼容基础版仅作为遗留代码等待替换，不能作为上线默认方案。
